...
const DEMO_EMAIL = "demo@zeofyou.app";
...
export default function Login() {
  ...
  const google = async () => {
    ...
    try {
      ...
      if (r.redirected) return; // browser navega fuera
      nav("/", { replace: true });
    } catch (e: any) {
      toast.error(e?.message || "Error inesperado con Google");
    } finally {
      setLoading(false);
    }
  };
  ...
  const demo = async () => {
    ...
    if (error) {
      const { error: upErr } = await signUp(DEMO_EMAIL, DEMO_PASSWORD, "Usuario Demo");
      if (upErr && !/registered/i.test(upErr.message)) {
        setLoading(false);
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-hero px-4 py-8">
      <GlassCard className="w-full max-w-md p-8">
        ...
        <div className="space-y-2">
          ...
          <Button variant="ghost" className="w-full gap-2 text-muted-foreground" onClick={demo} disabled={loading}>
            <Sparkles className="h-4 w-4" /> Entrar como demo
          </Button>
        </div>
